    ...
    async function _mfShiftPages(){
        ...
        if(!confirm('['+label+'] '+fromPage+'페이지부터 모든 항목을 '+shiftBy+'페이지 뒤로 밀겠습니다.\n계속하시겠습니까?')) return;

        var btn = document.querySelector('button[onclick="_mfShiftPages()"]');
        if(btn){ btn.disabled=true; btn.textContent='처리 중...'; }

        var raw = await _authFetch('manual_page_ranges/'+mode+'.json');
        if(!raw||typeof raw!=='object'){ alert('항목을 불러올 수 없습니다.'); if(btn){btn.disabled=false;btn.textContent='📄 페이지 밀기 실행';} return; }
        ...
        Object.entries(raw).forEach(function(e){
            ...
            var hit = false;
            if((r.start||0) >= fromPage){ newR.start = (r.start||0) + shiftBy; hit = true; }
            if((r.end  ||0) >= fromPage){ newR.end   = (r.end  ||0) + shiftBy; hit = true; }
            if(hit){ updates[key] = newR; changed++; }
        });

        if(!changed){ alert('기준 페이지('+fromPage+') 이상인 항목이 없습니다.'); if(btn){btn.disabled=false;btn.textContent='📄 페이지 밀기 실행';} return; }

        for(var k in updates){
            await _authFetch('manual_page_ranges/'+mode+'/'+k+'.json','PATCH',{start:updates[k].start, end:updates[k].end});
        }
    ...
    function _resetModalPos(contentId){
        ...
        c.style.width=''; c.style.height='900px'; c.style.maxHeight='900px';
    }
    (function(){
        ...
        _injectStyle('_modalFixedSize',
            '#noticeModalContent{width:min(95vw,1080px)!important;min-width:0!important;height:900px!important;max-height:900px!important;overflow:hidden!important;}' +
            '#urgentDetailContent{width:min(95vw,1080px)!important;min-width:0!important;height:900px!important;max-height:900px!important;overflow:hidden!important;}'
        );
    })();
